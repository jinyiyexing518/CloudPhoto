...
本地开发时，Vite 将所有 `/api/*` 请求代理到 `localhost:7071`。生产环境当前已部署的 `cloudphotos.top` 与 `www.cloudphotos.top` 都解析到 Nginx，优先走同源 `/api`；SWA 默认域名直达 Azure Functions。`cn.cloudphotos.top`、`global.cloudphotos.top` 与 `www` 智能 DNS 分流仍是规划配置，启用前不能作为可用入口。只读请求在线路网络或网关故障时双向回退；照片列表、动态视频、回收站和地理搜索等高成本读取不会仅因 5 秒内未返回响应头就向同一后端重放。写请求默认不重放；精确 `POST /auth/login` 因只执行凭据校验、幂等 `lastLoginAt` upsert 与无状态 token 签发，可在同源线路 transport failure 或 502/503/504/521-524 后串行尝试一次 Azure，绝不 hedge 并发。注册始终只提交一次：`www` 会先识别 Nginx/SWA 落点并选择同源 `/api` 或 Azure Functions，默认 SWA 域名继续固定走主域代理。Backend 发布会幂等确保主域、`www` 与精确 SWA 源都在 Function App CORS allowlist 中并拒绝通配。若注册表单 lazy chunk 被瞬时网络或浏览器策略拦截，失败只降级注册面板并提供“刷新新版”，登录表单继续可用；loader 同时处理普通 rejection 和 Vite 安全接管错误后产生的空模块，hover、focus 和 click 预载都不会再触发整个认证页错误边界。精确 `POST /photos/locations/recover` 不执行任何写入，也可串行回退和在 token refresh 后重放；其他 POST，包括上传、分享创建和位置修改，仍只调用一次。登录页同时监听已持久化和新到达的 PWA update-ready 状态，旧 Service Worker 客户端在未登录时也可通过“立即更新”显式激活等待版本；更新继续复用危险操作门禁，缺少 updater 或失败会显示错误。API 双线路竞速实现保留在独立 lazy chunk 中，登录的串行回退不加载该 chunk；已有会话恢复或用户提交登录/注册时会与认证工作区并行预热，避免把首个认证后请求的延迟转嫁给用户。
...
新上传照片的位置链路不依赖浏览器 MIME 完整性：空 MIME、`application/octet-stream` 和 JPEG/HEIC 非标准 MIME 会通过最多 64 字节文件头及扩展名识别，客户端 `exifr` 与服务端 legacy upload 都能独立恢复 GPS。客户端坐标必须成对、finite 且在纬度 ±90°/经度 ±180° 内，否则服务端先读取 TIFF/HEIC EXIF GPS；若照片只含 Adobe/Microsoft XMP 的 `exif:GPSLatitude/GPSLongitude`，再从已受 20 MiB 上传上限约束的同一正文解析单段或 Extended XMP 中的数值/度分秒与方向，并执行相同的原子范围校验。最终坐标写入 Blob metadata、同步 Cosmos 并随上传响应返回。同名缓存照片会采用最新 GPS、拍摄时间、SAS 与派生图，记忆地图随上传显式刷新。`GET /photos` 会在一次授权 scope 内批量读取 Cosmos 位置索引，只为 Blob metadata 中完全没有 GPS 键的历史照片补齐合法坐标；查询以 1.5 秒真实 wall-clock timer 与 SDK AbortSignal 同时限界，即使 SDK 重试忽略取消也会及时非致命降级。位置行可使用当前 `name` 或历史 `photoName` 标识，同一行同时提供且值不一致时拒绝，别名不能让孤儿或跨空间行通过授权交集。带 `sourceBlobEtag` 的索引必须匹配当前 Blob ETag，旧版无来源 ETag 的合法行继续兼容，任何 versioned 行存在时都不得回退到 legacy 行；当前 Blob GPS、单边/非法 metadata、跨空间行、孤儿、重复、非法坐标与已证实过期的索引均不会被覆盖。列表返回显式 `gpsMetadataPresent` 与当前 Blob ETag，位置接口同时返回 `scope`、`name`/`photoName` 与索引来源 ETag，因此地图只有在服务端确认两个 GPS metadata 键都不存在时才接受 Cosmos-only 历史位置，并使用同一来源版本与空间校验。若合法索引根本不存在，记忆地图会把这类已证明 metadata 完全缺失的当前照片按每批最多 64 张提交到只读恢复端点；服务端先验证 personal/group/admin 空间，再以当前 ETag 的 `If-Match`、顺序扫描、8 MiB 总 Range 预算和 2.5 秒真实 wall-clock deadline 读取原图内 TIFF/HEIC、标准或 Extended XMP GPS，避免并发大图同时占满预算却没有候选完成。deadline 从认证后的 group membership 与 96 KiB 请求体门禁开始，授权或解析停滞也不会无限占用请求。该路径不写 Blob metadata、Cosmos 或扫描标记；客户端只接受同名、finite/range 且 `sourceBlobEtag` 精确匹配当前照片的结果，并把它视为当前 Blob 权威坐标。单次地图会话最多检查 512 张当前照片，并受 64 个批次与 128 MiB 实际/保守读取量共同限制，避免 N+1 HTTP 与无界正文扫描；超时或预算未完成的批次只读重试，不会触发回填。时间线计数、无位置筛选和地图共用同一有限坐标对分类；当前有效 Blob GPS 与索引冲突时仍以 Blob 为准，用户可见的有/无位置集合互斥且总数闭合。持久化照片列表使用版本化 schema 路径，升级后不会恢复缺少补齐 GPS 的旧缓存。位置编辑按当前 workspace 与照片名重新解析目标，目标离开当前照片分区时会中止等待中的 PATCH 并安全关闭。反向地理编码失败显示“地址暂不可用”，不会被误报为无 GPS，也不会在状态文案中回显坐标。
...
个人和群组照片列表使用服务端目录游标分页：每页默认 24 张、最多 100 张，游标绑定授权 scope、目录 revision、不可变 snapshot 和最后一个稳定排序键。每次重建只写自己的 snapshot，Cosmos summary 通过 ETag 原子切换 active snapshot；同一 Blob 容器内按 scope 哈希保存的强一致 ETag fence 先后包围每次媒体 mutation、目录扫描和发布，跨 Functions 实例的分页读也必须在 Cosmos 查询前后看到同一 ready fence，且其 snapshot/revision 与 summary 完全一致。被取代或失败的重建不能覆盖当前目录行；每次兼容读取、成功发布及每个 revision 的后续首 page 会在同一 ready fence 下有界续清非 active snapshot，失败后由后续请求继续。目录就绪后，分页请求只查询同一 Cosmos partition 的 active snapshot，并仅为本页生成 SAS，不再扫描 Blob prefix、逐项读取属性或先签发整个图库。冷启动会立即显示最多 24 张仅含 thumbnail/preview 的只读预览和精确加载进度；完整数组到达前，筛选、周报、侧边栏统计、文件夹、重要片段、地图、胶囊与故事保持明确加载态。已有完整 SWR 缓存时继续显示旧完整图库，直到新完整结果原子替换；partial page 永不写入内存或持久化列表缓存。首次上线必须先发布 `writers-only` Backend，使所有媒体写入参与 fence 且分页请求继续回退完整数组；等待旧实例超过恢复窗口并核对健康后，再以独立提交切换到 `enabled`。首次访问尚未建立目录的 scope 才会兼容性执行一次完整列表并原子建目录；旧 Backend 返回数组时，新客户端也把它作为完整单页处理。
...
- **服务端照片目录分页** — 个人/群组目录按 scope partition、revision、不可变 active snapshot 与稳定时间键分页，每页默认 24 张；Cosmos summary ETag 负责原子换代，同容器 Blob ETag fence 负责跨实例 mutation/rebuild/read 一致性，旧重建与 session-stale Cosmos 读都无法发布旧目录；首次上线先经过 `writers-only` 排空旧实例，再独立激活分页，非 active snapshot 按固定上限可恢复清理；首批只读预览先显示，完整消费者与私有 cache 仅接受 exact-total 完整结果
...
- **照片卡片键盘主操作** — 时间线、历史回忆、重要片段和文件夹共用语义 `group` PhotoCard；独立原生按钮以文件名、媒体类型和日期命名，且内部只使用符合原生按钮 content model 的 phrasing content，普通模式 Enter/Space 打开 viewer，批量模式复用选择动作并通过 `aria-pressed` 表达状态。收藏/删除保持为非嵌套兄弟按钮和至少 44×44 命中区域；普通模式还可用 Shift+F10 或菜单键打开具名操作菜单，复用共享键盘模型支持方向键/Home/End 导航、Enter/Space 执行、Escape/外部点击关闭并安全恢复焦点，Tab 关闭后继续原生焦点顺序。删除确认复用共享 `alertdialog` 边界，默认聚焦取消、约束 Tab、支持 Escape/外部关闭，并在请求进行中保护关闭及恢复仍连接的删除或照片主按钮。文件名、日期和文件夹计数至少满足 4.5:1，操作图标、选择边界与焦点环至少满足 3:1；音频卡只渲染本地“语音备忘录”占位与类型徽章，不挂载原始或派生媒体 URL，同时继续复用同一键盘主操作、删除和操作菜单；批量选择或 busy 时隐藏并防御性阻止 GIF 播放控制，立即回到派生静态封面，不继续挂载原 GIF；视频封面修复、拖放与 touch 继续独立
...
- **统一照片路径授权** — 下载、元数据、移动、motion-video、Moments 统计、公开分享、缩略图、删除、恢复与永久删除统一验证个人所有权或群组成员关系；上传、文件夹改名与公开文件夹分享不能进入内部命名空间，managed share 打开时会重新验证持久化 scope/prefix 且只为普通照片签发 SAS；未知命名空间、内部缩略图/语音路径、跨空间移动和跨空间语音引用均 fail closed，列表也不会为无效历史语音指针签发 SAS
...
- **照片地点可靠恢复** — 照片 GPS 继续以 Blob metadata 为事实源；纬度/经度按有限数值和 `[-90,90]`/`[-180,180]` 原子校验，空白、`NaN`、Infinity、越界及单边值均进入恢复。维护仅对非删除图片做按 MIME 分层、每文件硬上限和每页 8 MiB 预算的 Blob range 扫描，并与上传共用 TIFF/HEIC + 标准/Extended XMP GPS reader；scan version 3 会重新检查此前被 TIFF-only 扫描标记为无位置的原图。完整无 GPS 才清理无效 pair 并删除陈旧索引，不完整扫描保留为可重试。已有合法 GPS 不下载原图并可幂等重建索引
...
- **本地自然日一致性** — 时间线分组、日期筛选、今日上传、时光胶囊和重要片段日统计共用本地日历 key；时光胶囊的“明天”通过本地日历加日计算，锁定倒计时通过本地 date-only 日历序号计算，不把固定 24 小时误当成夏令时下一个自然日。跨 UTC 与夏令时边界不会归入前一天；持久化的无效胶囊日期会被安全丢弃并提示，运行时异常值进入明确的无效解锁日期状态而不会显示 NaN
...
- **认证首屏窄屏稳定** — 登录/注册页在 320/390/500px 下由根文档承担长表单纵向滚动，不再产生固定横向溢出或 auth-page 双轴嵌套滚动；桌面装饰也保持在 1440px 容器边界内
- **认证控件一致可用** — 登录/注册页签、输入框、密码显示按钮、提交与安装入口统一继承系统字体并保持至少 44×44px 命中区；Edge/Chrome 即使使用英文区域设置，必填项与邮箱格式仍显示字段特定中文原生提示，输入后会清除旧提示
...
- **注册加载失败隔离** — 注册表单 chunk 的一次瞬时失败只显示注册专用降级面板并提供“刷新新版”，不会再让仍可工作的登录表单一起进入总错误页
...
- 目录与位置索引共用容器，但位置查询只接受未定义 `docType` 且经纬度为数值的文档；分页首屏的只读卡片会在授权 thumbnail、preview 及 direct/proxy 线路间有界回退
...
| Workflow | 文件 | 触发条件 |
|----------|------|----------|
| 部署后端 | `.github/workflows/deploy-backend.yml` | `packages/server/**`、共享算法运行时代码或后端发布/健康运行时代码变更 |
...
部署和更新日志同步 workflow 使用 **OIDC 认证**（无存储的 Azure 密码/密钥）；生产健康检查仅需仓库只读权限。Backend 非 `main` 手动运行使用独立、可取消的 concurrency group，不能替换 production pending；`main` 发布按 preflight → 无 OIDC build/package → final authorization → OIDC deploy/receipt 串行执行。两次授权都会刷新 `origin/main`，只要触发 SHA 之后出现任何 Backend artifact、根依赖清单、workflow 或发布/健康运行代码变更就硬停 stale run，并重新 dispatch 当前 `main`，单纯前端或文档前移不会误拦。该保护只适用于包含当前 workflow 的新 run；GitHub rerun 会使用原提交中的历史 workflow，因此本次上线前已有的 Backend run 不得 rerun，误触发时必须从当前 `main` 重新 dispatch。Frontend 对每次 `main` 前移都创建候选 run，保证被 ownership gate 接受的 SHA 必然是远端 main tip。GitHub concurrency 只保留一个 pending；若旧 SHA rerun 替换掉当前 Frontend tip 候选，stale ownership 分支会使用仅限该 production job 的 Actions write 权限重新 dispatch `main` production，避免跳过旧 run 后无人接替。
Backend build 与测试先使用根 `yarn.lock` 的 frozen graph；打包时只复制 server manifest 和同一 lock，在已由根安装填充的 Yarn cache 上执行 `--production --offline --frozen-lockfile`。随后逐项比较部署 `node_modules` 与测试图中的全部 `name@version`，任何缺少的直接依赖或未测试版本都会硬失败。Azure Windows 需要的 `sharp` 二进制 tarball 也必须匹配 `yarn.lock` 中的精确版本和 SHA-512 integrity，禁止重新引入无 lock 的 production `npm install`。
Backend deploy job 还会在自己的隔离 full-history checkout 中执行两次无 Actions write 的 target fence：一次在取得 Azure 身份前，一次紧邻 `config-zip` 前。这样 GitHub 只重跑失败的 deploy job 时也不能借用旧 artifact 跳过上游 authorize；若 main 在登录窗口出现 Backend-relevant 前移，旧 SHA 会在任何上传前 fail closed。
Azure 登录后、最终 target fence 前，deploy job 会幂等加入 `https://cloudphotos.top`、`https://www.cloudphotos.top` 与精确 SWA 默认域名三个 Function App CORS origin，并回读确认全部存在且没有通配 `*`；缺失或通配都会在代码上传前失败。部署包 `host.json` 固定同一非通配集合，避免本地/自托管 host 与 Azure 平台配置表达相反的认证边界。
前端 production push 与 `main` 上显式选择 production 的手动运行共用单一 concurrency group，运行中的 upload 不会被后续 run 取消；该串行边界之外，production job 在取 token 前、Azure upload 前和 upload 完成后执行 ownership fence：触发 SHA 必须仍是远端 main tip，并分页检查同 workflow/同 SHA所有 runs 的全部 attempts（包括同 run 的早期 attempt）的成功 upload+receipt。只有 upload 后 fence 仍成立才写 canonical receipt；若 main 在 upload 窗口前移，本 attempt 不会被 Health 认作 canonical，并立即重新 dispatch 当前 main。只有历史 receipt 对应的主域与 SWA 实时 no-store marker 也都等于当前 SHA 时才按重复部署跳过；历史 receipt 与线上漂移不一致时会重新发布修复。旧 SHA 和真实重复 SHA都以成功的 coalesced run 结束但 upload 为 0，因此不会顺序重复发布或制造 Azure Deployment Canceled failure。
PR 与 validation 使用独立 group，可取消旧验证。`workflow_dispatch` 默认 `validate`，当前 workflow 中非 `main` 分支与所有 PR 始终只构建和检查，upload 数量为 0。production job 只在 `main` 上通过 OIDC 从 Azure 即时读取 SWA deployment token，且当前 workflow 不引用 repository deployment token；要让仍含历史 workflow 的旧分支也无法取得生产凭据，必须按部署契约删除旧 repository secret。
Production Health 只把 Azure upload step 与紧随其后的 exact-SHA receipt 都成功的 Frontend/Backend run 当 canonical 部署事件；validation、旧 Frontend SHA和重复 SHA的 coalesced success 不产生实际 health verdict，Backend upload 成功但回读缺失则直接失败。classifier 通过 attempt-specific jobs API 读取事件对应的 `run_attempt`，避免重跑复用 run ID 时把一个 attempt 的 conclusion 与另一个 attempt 的 jobs 混合；Frontend Health concurrency 同样使用 run ID + attempt 隔离，使 non-deployment success 不能取消真实部署检查。`workflow_run` 先在隔离目录读取健康 workflow 自身版本的 classifier；通过分类后才把触发部署的 `head_sha` checkout 到独立验证目录，所有 contract/full-smoke 脚本与报告都绑定该 SHA，而不是可能已经前移的 `main`。分类器必须完整输出布尔 verdict，旧版或损坏输出会 fail closed；controller 再从自身可信版本执行 marker-only smoke，因此历史部署版本不能删除当前线上身份闸门。Frontend artifact 的 `deployment.json` 由主域、`www` 与 SWA 回读；Backend zip 根目录的同格式 marker 由主域 `/api/deployment`、`www` `/api/deployment` 与 Azure Functions 直连 `/api/deployment` 回读，全部必须 no-store、无重定向并精确匹配 triggering SHA。
前端构建配置使用 `packages/client/vite.config.mts` 和 Vite ESM Node API，静态契约会拒绝重新引入触发 CJS 弃用警告的 `.ts` 配置。构建产物契约同时要求 `AuthenticatedApp` 与 `PhotoGallery` 保持独立哈希 chunk，防止工作区代码重新进入未登录首屏。

### 生产健康检查

`node scripts/production-smoke.mjs` 使用 Node 24 内置 `fetch` 独立检查权威 DNS 已部署的主域、`www` 和 Azure 直连。主域与 `www` 都必须返回携带完整安全头的 Cloud Photo HTML、有效 `/healthz` 路由身份、未登录 `/api/auth/me` 401 和 `/api/changelogs` JSON 数组；Azure 直连继续独立检查前端与 API。认证检查额外以固定不存在账号执行无副作用登录、以空对象触发注册校验，并分别模拟 `www`→Functions 与 SWA→主域代理的浏览器 JSON POST 预检，要求 exact ACAO 与 `Content-Type` allow-header；不会创建账号或使用真实凭据。任一受检 URL 返回重定向都会失败，不能借用另一入口的成功响应制造假绿。主域和 Azure 的 manifest、180px Apple Touch PNG 及随机缺失 hashed JS/CSS 仍分别验证，避免重复下载不能提供额外隔离证据的同一上游静态资源。普通定时或手动轮次执行 23 个检查；Frontend deployment 事件追加三条 Frontend marker，Backend deployment 事件追加三条 Backend marker，各自 full smoke 都是 26 项，并各由 controller 再独立执行 3 项 marker-only receipt。所有检查并行执行，完成后仍按固定顺序输出目标、状态和耗时；每个请求 10 秒超时，失败后最多重试 8 次、轮次间隔 15 秒，以覆盖前后端部署传播竞态。最坏检查时长仍为 185 秒（8 × 10 秒并行轮次 + 7 × 15 秒等待，不含 runner setup），低于 workflow 的 10 分钟上限。Frontend production、Backend 与普通定时/手动检查使用相互隔离的成功并发组：同类 deployment 更新会取消旧轮次，但普通、后端或 Frontend validation 检查不能取消前端 SHA 验证；Frontend 非部署完成事件与失败部署按 run ID + attempt 隔离，其显式结果不会被同一 run 的重跑或后续成功事件覆盖。

默认入口为 `https://cloudphotos.top`、`https://www.cloudphotos.top`、`https://brave-sand-053b07a00.7.azurestaticapps.net` 和 `https://cloudphoto-api.azurewebsites.net/api`。可通过 `PRODUCTION_BASE_URL`、`PRODUCTION_WWW_BASE_URL`、`PRODUCTION_AZURE_FRONTEND_URL`、`PRODUCTION_AZURE_API_BASE_URL` 覆盖入口，或使用对应的 `PRODUCTION_HOME_URL` / `PRODUCTION_HEALTH_URL` / `PRODUCTION_AUTH_ME_URL` / `PRODUCTION_AUTH_LOGIN_URL` / `PRODUCTION_AUTH_REGISTER_URL` / `PRODUCTION_CHANGELOGS_URL` / `PRODUCTION_DEPLOYMENT_URL`、`PRODUCTION_WWW_*` 与 `PRODUCTION_AZURE_*` 变量覆盖单个检查地址或预检 origin；合法 40 位 `PRODUCTION_DEPLOYED_SHA` 启用 Frontend marker，`PRODUCTION_BACKEND_DEPLOYED_SHA` 启用 Backend marker，`PRODUCTION_SMOKE_SCOPE=deployment|backend-deployment` 可只执行对应三条身份回读。运行 `yarn test:production-smoke` 可在本机用内置 HTTP fixture 重复验证成功、重试和失败路径，无需访问生产环境。
