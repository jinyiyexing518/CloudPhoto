...
本地开发时，Vite 将所有 `/api/*` 请求代理到 `localhost:7071`。生产环境当前已部署的 `cloudphotos.top` 与 `www.cloudphotos.top` 都解析到 Nginx，优先走同源 `/api`；SWA 默认域名直达 Azure Functions。`cn.cloudphotos.top`、`global.cloudphotos.top` 与 `www` 智能 DNS 分流仍是规划配置，启用前不能作为可用入口。只读请求在线路网络或网关故障时双向回退；照片列表、动态视频、回收站和地理搜索等高成本读取不会仅因 5 秒内未返回响应头就向同一后端重放。写请求默认不重放，唯一例外是精确 `POST /auth/login`：同源线路 transport failure 或 502/503/504/521-524 后才串行尝试一次 Azure，绝不 hedge 并发。登录只执行凭据校验、幂等 `lastLoginAt` upsert 与无状态 token 签发，因此这次有界串行补偿不会产生业务重复；注册、上传、分享创建及其他写入仍只调用一次。登录页同时监听已持久化和新到达的 PWA update-ready 状态，旧 Service Worker 客户端在未登录时也可通过“立即更新”显式激活等待版本；更新继续复用危险操作门禁，缺少 updater 或失败会显示错误。API 双线路竞速实现保留在独立 lazy chunk 中，登录的串行回退不加载该 chunk；已有会话恢复或用户提交登录/注册时会与认证工作区并行预热，避免把首个认证后请求的延迟转嫁给用户。
