...
- **原子且可恢复的文件夹重命名** — 仅允许同级改名；服务端先完整检查源/目标前缀，再以目标端 `ifNoneMatch=*` 和源 ETag 条件复制全部原图与派生媒体，copy 后复核完整 inventory，删除每个源前以短租约锁住并核验对应目标的 copyId + final ETag，再按源 ETag 条件删除。目标已存在时拒绝覆盖/合并；复制失败只回滚仍可证明归属本操作的目标，任何并发变化或部分删除都保留至少一份并明确报告
