...
上传使用网络感知的加权并发：4G/快网预算 3，未知网络或 3G 预算 2，`saveData`/2G 预算 1；图片权重 1，视频或大文件权重 2，因此快网最多同时 3 张图片或 1 个视频 + 1 张图片，未知网络仍可并发 2 张小图。暂停只冻结新任务，已开始的 XHR 继续完成；总进度按已完成文件字节与所有活跃 XHR 的 loaded 字节聚合。仅网络错误、超时、408/425/429/5xx 使用稳定 `uploadId` 重试，并遵守 `Retry-After` 与 60 秒上限的指数 full jitter；其余 4xx 立即显式失败。
...
- **网络感知的有界并发上传** — 4G 最多并行 3 张图片或 1 视频 + 1 图片，未知/3G 并行 2 张小图，省流/2G 串行；聚合字节进度、暂停新任务、稳定 uploadId 和状态型重试保持部分失败可对账
...
- **Header 菜单完整键盘访问** — 空间切换与用户菜单使用一致的 menu 键盘模型，支持方向键、Home/End、Escape、Tab 和焦点恢复；禁用项会被跳过，受保护的空间切换不会改变当前空间或意外关闭菜单
- **群组弹窗焦点边界** — 新建群组与群组设置提供标准 dialog 语义、动态 Tab/Shift+Tab 循环、嵌套弹窗隔离及 connected-only 焦点恢复；保存、邀请或删除进行中时不能通过遮罩或 Escape 提前关闭
