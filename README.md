...
部署和更新日志同步 workflow 使用 **OIDC 认证**（无存储的 Azure 密码/密钥）；生产健康检查仅需仓库只读权限。后端共享算法仍只在运行时代码/清单变化时部署；Frontend 对每次 `main` 前移都创建候选 run，保证被 ownership gate 接受的 SHA 必然是远端 main tip。GitHub concurrency 只保留一个 pending；若旧 SHA rerun 替换掉当前 tip 候选，stale ownership 分支会使用仅限该 production job 的 Actions write 权限重新 dispatch `main` production，避免跳过旧 run 后无人接替。
...
Production Health 只把 `Deploy to Azure Static Web Apps` step 实际 started 且后续 canonical receipt 成功的 Frontend run 当部署事件；validation、旧 SHA和重复 SHA的 coalesced success 不产生实际 health verdict。classifier 通过 attempt-specific jobs API 读取事件对应的 `run_attempt`，避免重跑复用 run ID 时把一个 attempt 的 conclusion 与另一个 attempt 的 jobs 混合；Frontend Health concurrency 同样使用 run ID + attempt 隔离，使 non-deployment success 不能取消真实部署检查。`workflow_run` 先在隔离目录读取健康 workflow 自身版本的 classifier；通过分类后才把触发部署的 `head_sha` checkout 到独立验证目录，所有 contract/full-smoke 脚本与报告都绑定该 SHA，而不是可能已经前移的 `main`。分类器必须完整输出布尔 verdict，旧版或损坏输出会 fail closed；controller 另以 marker-only smoke 同时检查主域、`www` 与 SWA，固定执行线上 SHA guard，因此历史部署版本即使没有新 smoke 逻辑也不能假绿。前端生产 artifact 同时写入仅含 commit SHA 的 no-store `deployment.json`，Production Health 要求主域、`www` 与 SWA 直连都精确返回该 SHA。
