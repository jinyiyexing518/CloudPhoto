...
- **传输进度横幅** — 上传、下载、批量 mutation 与历史维护任务共用顶部固定横幅；维护任务总量未知时只显示准确累计计数，不伪造百分比
...
- **PWA 更新传输守卫** — 上传/下载/批量删除/语音备注录制或上传/批量 mutation/历史维护任务进行中时，更新按钮禁用并提示任务完成后更新；同一守卫阻止切 Tab、切群组和关闭页面
...
- **传输安全守卫** — 上传/下载、批量删除、语音、批量 mutation 与历史维护任务进行中时阻止 Tab/空间切换，浏览器刷新或关闭显示 unload 确认
