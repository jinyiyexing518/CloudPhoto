...
- **历史照片元数据回填** — 一键扫描所有缺少拍摄时间或 GPS 的旧照片，从 EXIF 自动补全；服务端按绑定账号空间的不透明游标分批且每次最多读取一个 Blob 页，写回前以 ETag 合并并从最新 Blob 状态同步位置缓存，避免覆盖并发编辑、删除或缩略图字段；切换账号会立即终止后续批次
...
`node scripts/production-smoke.mjs` 使用 Node 24 内置 `fetch` 同时检查主域名和 Azure 直连：两个前端均返回 Cloud Photo HTML、两个未登录 `/api/auth/me` 均返回 401，且两个 `/api/changelogs` 均返回 200 JSON 数组。同一轮的 6 个检查并行执行，完成后仍按固定检查顺序输出目标、状态和耗时；每个请求 10 秒超时，失败后最多重试 8 次、轮次间隔 15 秒，以覆盖前后端部署传播竞态。最坏检查时长为 185 秒（8 × 10 秒并行轮次 + 7 × 15 秒等待，不含 runner setup），低于 workflow 的 10 分钟上限；若触发它的部署失败，workflow 会直接失败而不会将线上状态误报为健康。
