...
媒体也使用双线路：派生图继续用无响应体的 `HEAD` 在 Blob 直连和 `/media` 代理中选择线路；视频打开时则用 `Range: bytes=0-1` 单独验证持续流所需的 byte-range 能力，只接受 `206` 并立即取消响应体，代理返回 `200` 时优先直连。播放中的 `waiting` / `stalled` 只有在可见、未暂停、未 seek、未结束且 4 秒没有时间进展时才切换一次线路，并在 metadata 恢复后还原位置、播放意图、静音、音量和倍速。跨域 Blob 备用线路不强制 CORS 模式，避免有效的 `206 video/mp4` 被浏览器误判为格式错误；同源代理仍可安全执行封面截帧。两条线路耗尽后明确显示失败与重试，不会永久转圈或在直连/代理间循环；所有非 `206` 的 Range 响应都会主动取消响应体，避免忽略 Range 的线路继续下载完整视频。
...
- **历史视频封面自修复** — 仅接近视口且缺少、加载失败或内容近乎纯白/纯灰的派生图在浏览器空闲时进入全局去重队列；未知网络并发 1、明确快速网络最多 2，省流量/离线/2G 自动暂停，并受 48 MiB 单文件与 160 MiB 会话估算预算约束。修复会在视频稍后位置采样多个候选帧、选择信息量最高的画面，再复用既有 ETag 持久化端点，刷新后所有页面直接使用静态 WebP
...
- **照片卡片键盘主操作** — 时间线、历史回忆、重要片段和文件夹共用语义 `article` PhotoCard；独立原生按钮以文件名、媒体类型和日期命名，普通模式 Enter/Space 打开 viewer，批量模式复用选择动作并通过 `aria-pressed` 表达状态。收藏/删除保持为非嵌套兄弟按钮和至少 44×44 命中区域，GIF 控制、视频封面修复、拖放、touch 与右键菜单继续独立
...
- **角色感知的私有缓存** — 照片列表与媒体缓存按 `用户 + 角色` 授权快照管理；管理员降权、切号或注销会立即清理旧列表和 SAS 媒体
...
- **无障碍工作区页签** — 六个主视图使用具名 WAI-ARIA tablist/tab/tabpanel 关系、稳定 ID 与 roving tabindex；左右方向键循环自动激活，Home/End 跳到首尾，Enter/Space 保留原生按钮行为。所有入口继续复用统一切换与 mutation guard，拒绝切换时 selected 和焦点留在原页签；弹窗打开时不允许后台页签响应，移动端聚焦页签仅在横向条内滚动到可见位置
...
- **窄屏悬浮操作收纳** — ≤360px 默认只显示 safe-area 对齐的 48px 快捷入口，展开后保留筛选/整理、最近上传和整理诊断动作，并支持 Escape、焦点回归与 `aria-expanded`
...
- **自动隐藏 Header 保持焦点可见** — 空间或头像触发器获得焦点、任一 Header 菜单打开、或菜单发起的弹窗仍活跃时会立即显示并锁定 Header；共享弹窗边界只向仍连接且可见的触发器恢复焦点，Tab 自然离开 Header 后才允许后续向下滚动再次隐藏
...
- **时光胶囊有界照片选择** — 新建胶囊只挂载首批 18 张派生图，并以照片区自身为 IntersectionObserver scroll root 每次追加 12 张；切换来源或空间会重置窗口，完整选择 Set、计数与提交结果不受渲染窗口影响，视频始终只显示静态派生封面或占位
- **自动故事语义进度轴** — 任意照片数量都只渲染一个原生 range scrubber，支持方向键、Home/End、点击与拖动跳转并同步自动播放、前后切换和计数；关闭、卸载或快速连续导航会清理 200ms 过渡任务
