...
- **传输进度横幅** — 上传、下载和批量 mutation 共用顶部固定横幅；批量操作显示具体类型、`done/total`、失败数和百分比
...
- **PWA 更新传输守卫** — 上传/下载/批量删除/语音备注录制或上传/批量 mutation 进行中时，更新按钮禁用并提示“传输完成后更新”；同一守卫阻止切 Tab、切群组和关闭页面
...
- **传输安全守卫** — 上传/下载、批量删除、语音与批量 mutation 进行中时阻止 Tab/空间切换，浏览器刷新或关闭显示 unload 确认
