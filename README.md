...
- **登录首屏分包** — 未认证访客只加载鉴权壳；工作区与图库按认证意图并行预载，入口 JS 从 179.82 kB 降至 28.84 kB（约 -84%），首屏 CSS 从 128.56 kB 降至 9.38 kB
...
- Service Worker 首装只预缓存 HTML、9.38 kB 登录样式、28.84 kB 入口 JS、React 与注册运行时（183.31 KiB）；工作区 JS/CSS 与图库等动态 chunk 首次使用后进入 `app-code-v1`，相较原始 894.44 KiB 首装资源减少约 80%
