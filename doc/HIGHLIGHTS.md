...
- **注册资源失败不再拖垮登录** — 注册 lazy chunk 被网络或浏览器策略瞬时拦截时，loader 对 reject 和 Vite `preventDefault()` 后的 fulfilled-undefined 都解析为注册专用 fallback；登录 tab 继续可用，用户可通过统一的“刷新新版”恢复
- **群组管理按意图加载** — 新建群组和群组设置从认证工作区初始包拆为独立 lazy chunk，hover/focus 复用缓存 Promise 预热；加载或跨部署失败时仍可关闭、按 Esc 退出并保持焦点隔离。`AuthenticatedApp` 从 167.61 kB / gzip 54.65 kB 降至 161.05 kB / gzip 52.97 kB
...
- **PWA 最小首装缓存** — Workbox 从预缓存全部 894.44 KiB 资源改为只安装约 195.21 KiB 应用壳（约 -78%，包含用于发现后续部署的 update policy）；动态工作区 JS/CSS、注册/群组对话框与图库首次访问后进入 `app-code-v1` CacheFirst 缓存，兼顾首屏带宽与后续离线复用
- **Header 空间回收** — 已登录 Header 不再常驻 PWA 安装按钮，避免挤压群组切换、照片数量与用户菜单；安装能力继续保留在登录页、用户菜单和「设置 → 应用」
- **侧栏筛选响应式与认证日期一致性** — 同一 `FilterBar` 以显式 sidebar variant 隔离抽屉布局，搜索/清空先行、快捷筛选与网格尺寸按容器自动换行；320–480px 与 200% 缩放下长标签和激活 chip 不再越界，所有交互保持至少 44px，真实 320px 视口的 262px 抽屉也量化为 224/224px 筛选行且控件边界完整。抽屉使用 dialog 焦点闭环，打开聚焦关闭入口、关闭恢复 launcher。筛选器、拍摄时间编辑和时光胶囊复用原生控件字体/高度规范，保留系统 date picker 与 `YYYY-MM-DD` 值；胶囊日期按本地日历 key 生成，跨 UTC 日界线也不会提前或延后一天。时间线、卡片和文件夹查看器通过单一显式 `zh-CN` helper 输出稳定中文日期并安全处理无效值。390px 触屏仅扩大照片收藏/删除和头像命中区到 44×44px，保证不重叠、准确命中与可见焦点；320/360px 文件夹网格改为单列，动作不再被窄卡片裁切。WorkspaceFab 在 320–480px 默认折叠为 48px 启动器，展开不越界，动作执行前先把返回焦点交给可见启动器再收起，避免新界面捕获隐藏按钮。其他 ≥24px 紧凑控件不膨胀，宽桌面默认样式不变
- **PWA 安全更新闸门** — `onNeedRefresh` 仅设置全局 `update-ready` 状态并发事件，不自动 `updateSW(true)`/刷新页面；登录页期间收到更新事件也会在进入工作区后恢复更新提示
- **跨部署资源保留 + chunk 一次性自愈** — 发布产物按 SHA-256 保留最多 **24 代 / 64 MiB** 的完整历史 hashed JS/CSS，旧 active SW 缓存的 app shell 无需加载新 bootstrap 即可恢复；最旧代次整体淘汰，source map/摘要漂移/撤销代次 fail closed，随机缺失 JS/CSS 返回 404 JSON 而非 SPA HTML。超出窗口后，pre-React 入口只识别同源 content-hashed dynamic import/preload 失败，安全会话自动 reload 上限 **1 次**，危险操作期间 reload/worker activation 为 **0**；时间线、文件夹、重要片段、地图、胶囊与故事各有 keyed ErrorBoundary，把单个 lazy chunk 的故障域缩至 **1 个 panel**。双标签 waiting worker、standalone 与 old→new deploy 由隔离 Edge/Chromium profile 验证，sessionStorage 只留 opaque 指纹和 allowlisted tab，生产 DOM 中 raw URL/stack 为 **0**
- **前端 production main-tip 幂等** — 事故中同 SHA 的两个 SWA upload 竞速后留下 1 success + 1 Azure Deployment Canceled failure。现在每次 main push 都创建候选 run，production group 不取消在途 upload；token 前、upload 前、upload 后三段 ownership fence 要求 SHA 仍是远端 main tip，并扫描同 SHA 所有 runs/attempts 的成功 receipt 与主域/SWA实时 marker。旧 SHA和真实重复 SHA upload 为 **0**；upload 窗口内 main 前移则不写 canonical receipt并重排当前 tip，线上漂移会修复发布；旧 rerun 若替换唯一 pending，也会重新 dispatch 当前 main 以避免 orphan。Health 只认 attempt-specific Azure upload + canonical receipt，不把 validation/coalesced success 伪装成部署 verdict。PR 与 validation upload 为 **0**
- **后端 production exact-SHA 闭环** — 非 main run 与 production pending 隔离；preflight → 无特权 build/package → final authorization → 唯一 OIDC deploy 串行执行，完整历史两次拒绝并 requeue 被后续 Backend-relevant commit 取代的 SHA，无关前端/文档 commit 不误拦。zip 携带 canonical `deployment.json`，跨 job artifact 在登录 Azure 前复核；Azure 返回成功后必须由主域、`www` 和 Functions 直连 **3/3** no-store marker 精确回读。Production Health 只接受真实 attempt-specific upload+receipt，并从 deployed revision 执行 **26** 项 full smoke、从 controller 再执行 **3** 项身份闸门。上线前 run 仍使用其历史 workflow，不得 rerun；误触发后从当前 main dispatch 恢复
- **后端 partial rerun 防回滚** — OIDC deploy job 以独立无凭据 full-history checkout 在 Azure login 前与 `config-zip` 前执行 **2 次** read-only target fence；GitHub 只重跑失败 job 也不能跳过 freshness check 或复用旧 artifact 覆盖新 Backend。Actions write 仍只留在 unprivileged preflight/authorize，部署身份与自动 requeue 权限不合并
- **后端 tested=deployed 依赖闭环** — server tests 与 production-only deploy-stage 共同读取根 frozen `yarn.lock`；离线打包后遍历全部嵌套 package identity，要求每个部署 `name@version` 都来自测试图。Windows sharp tarball 另以 lock 中的精确版本和 SHA-512 integrity 验证，无 lock caret 漂移、缺失直接依赖或未测试传递版本均在 Azure 登录前硬失败
...
- **部署 SHA 闭环验收** — Frontend artifact 写入只含 commit SHA 的 no-store marker；Production Health checkout triggering `head_sha`，并要求主域、`www` 与 SWA 直连产物精确匹配且拒绝重定向，杜绝 main 前移、入口漂移或借用其他入口响应造成的假绿
