...
- **视频中途卡死自恢复** — 视频线路用 2-byte Range 严格验证 `206`；播放态连续 4 秒无进展才在 Blob 与 `/media` 间有限换线，恢复原时间点、播放意图、音量、静音与倍速。跨域 Blob 备用线路不强制存储 CORS，pause/seek/切后台不会误切，两条线路都失败时结束 loading 并提供原位重试
