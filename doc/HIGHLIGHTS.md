...
- **前端 production main-tip 幂等** — 事故中同 SHA 的两个 SWA upload 竞速后留下 1 success + 1 Azure Deployment Canceled failure。现在每次 main push 都创建候选 run，production group 不取消在途 upload；initial/final ownership 双检要求 SHA 仍是远端 main tip，并扫描同 SHA 所有 runs/attempts 的成功 receipt 与主域/SWA实时 marker。旧 SHA和真实重复 SHA upload 为 **0**，线上漂移会修复发布；旧 rerun 若替换唯一 pending，会重新 dispatch 当前 main 以避免 orphan。Health 只认 attempt-specific Azure upload + canonical receipt，不把 validation/coalesced success 伪装成部署 verdict。PR 与 validation upload 为 **0**
