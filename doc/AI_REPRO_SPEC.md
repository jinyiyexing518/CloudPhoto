...
2.67 PhotoCard 键盘与触控契约：timeline 日期组、历史回忆、重要片段、moments 与 FolderView 必须复用同一 PhotoCard。外层使用无 click/key handler/tabIndex 的语义 `article`，主打开/选择动作只能由独立 `button type="button"` 承载且不得包裹收藏、删除、移动、GIF 控制或其他 action。主按钮在普通模式依赖原生 Enter/Space 调用既有 viewer onClick，在 batch selection 模式调用既有 onSelect，并以 `aria-pressed` 表达 selected；busy 时真实 disabled，不得同时在 thumbnail/wrapper 绑定第二套 click/key handler。主按钮 aria-label 必须包含原文件名、照片/视频/GIF/动态照片类型和可用拍摄或上传日期，空名使用「(未命名照片)」，不得拼入 URL、SAS、thumbnail 或 preview。收藏与删除维持独立兄弟操作、事件隔离、至少 44×44 hit box 和明确 focus-visible，不得绝对定位成彼此重叠；主按钮 focus ring 必须在 overflow-hidden 卡片内可见。选择 badge 不截获指针；GIF 播放/暂停位于主按钮之上且继续 stopPropagation。右键菜单、touch、double-click、视频封面 repair、拖放与 context actions 不得回归；source contract 必须证明 PhotoGallery 的 timeline/moments/insight 与 FolderView surface 均使用共享组件。
