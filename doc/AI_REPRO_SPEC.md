...
2.97 注册 lazy chunk 失败隔离契约：`AuthPage.tsx` 必须继续通过同一个缓存 Promise 和 `React.lazy` 按 hover/focus/click intent 加载独立 `RegisterForm` chunk，不得将注册字段、校验或提交逻辑静态带回登录入口。动态 import rejection 必须在 loader 内部消化并交给 `reportLazyBoundaryFailure`，使预载失败不会成为未处理 rejection 或越过 `AuthPage` 触发整个“登录与注册”错误边界；同一失败 Promise 只上报一次。loader 必须解析为与真实注册表单同契约的本地 fallback module：inactive 时保持隐藏，active 时只替换注册 panel、显示注册专用错误，并通过现有 `requestDeploymentRefresh` 提供“刷新新版”；登录 tab、输入与提交保持可用。`auth-layout-contracts.test.mjs` 必须执行真实 transpiled loader 并注入 rejected chunk，证明两次调用复用同一 fallback、错误只上报一次、inactive 隐藏和 active 恢复按钮；静态契约与构建检查继续证明唯一 RegisterForm deferred chunk、无 HTML preload/SW precache，并维持不可放宽的 36,000 B raw 登录入口门禁。
