...
2.65 已认证首屏媒体边界：selection-ready 必须绑定当前授权用户；持久化群组选择完成成员验证前，照片工作区不得读取个人列表缓存、渲染个人卡片或请求无 `groupId` 的 `/photos`，恢复后只允许请求该群组；明确 personal 可独立 resolve，群组 API 失败时仍可恢复。照片 effect 依赖稳定 resolved workspace ID，`groupsLoaded` 后续变化不得重启 personal 请求。同工作区成功刷新后 5 分钟内 focus/visibility/视图返回不得重复拉取页面序列，进行中的分页不得被 focus 重启。时间线、历史片段、故事、地图详情与文件夹网格仅使用授权 `thumbnailUrl`/`previewUrl`，旧缓存对象缺少 derivative 时显示本地占位，不得 fallback 到 `url`。打开 viewer 后可按 thumbnail→preview 策略显示，原图仅由显式原图预览、动图播放或下载触发；视频无封面继续显示本地占位且不得挂载原视频。行为 fixture 必须量化恢复刷新 personal request count=0、首次稳定渲染 personal card count=0、初始 grid original-media request count=0、同工作区 in-flight restart count=0；源码和构建产物同时保留策略契约。大型图库统一遵循 2.94 的 server-backed catalog 分页：cold load 先挂载 bounded derivative-only 只读预览，所有完整数组消费者、mutation UI 和私有列表缓存必须等 exact-total 完成，禁止只给 Timeline 做半分页。已登录 Header 继续禁止 `header-install-button`、对应 CSS 与顶部安装文案。
...
2.74 生产 HSTS 与入口真实性契约：SWA `globalHeaders` 和 `infra/nginx.conf` 所有本地 HSTS 必须严格为 `max-age=31536000; includeSubDomains; preload`；Nginx 前端代理必须隐藏上游 `Strict-Transport-Security`、`X-Content-Type-Options`、`X-Frame-Options` 后保留本地安全头，不得改写 SWA Cache-Control，也不得弱化 `/api`、`/media` 的 CORS/Range/缓存。production smoke 必须把权威 DNS 已部署的 `cloudphotos.top`、`www.cloudphotos.top` 与 SWA 默认域名作为独立 target：SWA 必须只返回唯一 canonical HSTS，两个代理入口的第一个 effective HSTS 必须 canonical；短 max-age、缺 directive 或非 canonical 首值必须失败。VM 模板未热加载时只允许尾部出现已知旧本地值 `max-age=31536000; includeSubDomains` 或重复 canonical，`max-age=0` 等任意其他尾值必须失败；该情况作为 non-blocking infrastructure drift，且不得声称重复已消除。主域和 `www` 都必须验证首页/安全头、`/healthz`、未登录认证和 changelog；Frontend deployment 还必须要求两者及 SWA 的 no-store marker 都等于 triggering SHA；任一受检 URL 的 3xx 必须失败，不得通过重定向借用其他入口响应。权威 DNS 事实必须分成当前部署与规划：`dns23.hichina.com` 上 apex/www 为 `A 20.195.27.151`，cn/global 为 NXDOMAIN；在 DNS 提供商真正配置前，不得把 cn/global/智能 DNS 写成已上线入口或健康 target。GitHub 前端部署只更新 SWA，不得尝试未知 SSH、提交 secret 或伪装自动 VM 部署。
...
2.94 服务端照片目录分页契约：个人与群组 scope 必须在现有 `photoLocations` Cosmos 容器和 `/scope` partition 中，以 `photo-catalog`、`photo-catalog-summary`、`photo-catalog-mutation` 文档隔离目录；跨实例权威状态复用现有媒体 Blob 容器的内部 `_photo-catalog-fence/{sha256(scope)}.json`，不新增基础设施，且该路径不得公开签名或进入普通照片列表。地图查询必须保留数值 `lat/lon` 过滤。每个 rebuild 使用唯一 `snapshotId`，行 id 为 `snapshotId + Blob name` 的 SHA-256 base64url；Cosmos summary 只通过当前 ETag 原子切换 `activeSnapshotId`，page query 必须同时过滤授权 scope 与 active snapshot。被取代、超时或失败的 rebuild 即使迟到写行，也不得覆盖、删除或被 page 返回；其 finally 只能清理自己的 snapshot，成功发布后才可清理先前 active snapshot。稳定降序 sortKey 依次使用 `takenAt`、`createdAt`、Blob `lastModified`，再拼接 Blob name base64url；个人/群组完整兼容路径与 page path 必须使用同一 sortKey。`GET /photos?limit=N&cursor=...` 默认 24、最大 100，返回 `{items,nextCursor,done,revision,total}`；不透明 cursor 必须绑定 version、授权 scope、catalog revision、active snapshot 与最后 sortKey，非法 cursor 为 400，revision 漂移为 `stale-photo-cursor`，完成时唯一项数必须严格等于 total。Cosmos NoSQL 允许 `TOP @take` 参数化，测试替身不得以关系型 SQL 限制误判该合法语法。page 热路径只能执行两次有界 Blob fence 读、同 partition Cosmos keyset query 和本页 SAS 签发，不得 Blob listing、逐项 HEAD 或整库 SAS。每个 page/complete read 必须先取得 ready Blob fence，再读取 Cosmos summary 并要求 `snapshotId/revision` 完全一致；行查询完成后必须重读并要求 fence ETag 未变，普通 Cosmos Session consistency 或重复 summary read 不能替代该协议。目录缺失时客户端只允许一次无 limit 的 legacy materialization：rebuild 必须先通过 Cosmos summary CAS，再在 `listBlobsFlat({includeCopy:true})` 前取得无活动 mutation token 的 Blob rebuild owner；扫描与分批写 snapshot 行时必须用 heartbeat 同时续租 Cosmos summary ETag 和 Blob owner，失去任一 lease 即停止。扫描遇到 `copyStatus=pending` 必须阻断发布，terminal failed/aborted copy 不得进入目录。结果写入隔离 snapshot 后，先以最新 rebuild ETag 条件发布 Blob ready marker，再以最新 summary ETag 发布 Cosmos summary。Cosmos 404/412 必须条件回滚自己的 ready marker；响应不确定只能在精确 read-back 证明同 snapshot/revision 已 ready 后报告成功。旧客户端省略分页参数继续收到数组，新客户端命中旧 Backend 数组时将其作为完整单页，管理员无 groupId 的跨 personal-partition 查询继续走 legacy。每个已知 Blob mutation 在首次媒体写入前建立 Cosmos journal、使 summary unready，并以 Blob ETag CAS 将唯一 token 加入集合；并发 mutation 不能使用 boolean、计数或单 token。每次后续 Blob 写前必须续租并验证自己的 token，失去 token 的暂停 writer 不得恢复写入。单照片 move 的 server-side copy 必须用总 deadline 中止 stalled poll，向 start/poll 传 abort signal，每个 copy/poll/lease/abort/delete 前续租，以 `If-None-Match:*` 禁止覆盖，并复制完整 source metadata 加内部 operation marker；begin response 丢失、deadline 或 terminal failure 只按 marker/copyId/ETag 删除本次目标。copy 成功后必须用至少 15 秒目标 lease 和 10 秒以内 critical section 复核 copyId/status/ETag，再按 source ETag 条件删除；删源返回 404/false 表示 source 已不存在且目标必须保留，不能因重试后的 404 删除唯一副本。source 412 才可在释放 lease 后条件清理未变目标，外部改写目标必须保留并返回显式 recovery-required。完成/失败后必须先以 Blob CAS 只移除自己的 token且保持 ready 为空，再次 invalidation Cosmos summary 并删除 journal；旧 mutation 的迟到 finish 必须使后来发布的目录重新失效。媒体结果已明确成功后，catalog finish 失败只能记录并保持目录 fail-closed，不能覆盖原响应而诱发不可逆 mutation 重试。活动 token/rebuild owner 分别返回 `photo-catalog-mutating` / `photo-catalog-rebuilding`，不得 fallback 到 live Blob scan；近期 malformed、noncanonical 或任一未来时间戳 Blob fence fail closed，只有 Blob `lastModified` 证明超过 10 分钟恢复窗后才能条件重建。Cosmos journal/owner 时间必须是 canonical ISO；malformed/future/missing timestamp 或 timestamp 已过期但 server record 仍新鲜均 fail closed，只有 timestamp 与服务端 `_ts` 都超过对应恢复窗时才能接管。客户端每页都校验 auth generation、workspace、cache generation、revision、total、cursor progress 和 name dedupe；stale cursor 最多从第一页重启一次。cold load 首 page 最多挂载 24 个 thumbnail/preview 只读卡片并显示 loaded/total，筛选、周报、侧栏统计、文件夹、Moments、MemoryMap、Capsule、Story 和 mutation UI 在完成前 gated；warm complete cache 保持完整 UI 直到新 exact-total 数组原子替换，partial page 永不写内存或 Cache Storage。`photoCatalog.test.mjs`、`movePhotoSafety.test.mjs`、`renameFolderSafety.test.mjs`、`test-photo-loading-behavior.mjs` 与 `check-photo-loading-contracts.mjs` 必须共同锁定排序、游标、跨页去重/闭合、不可变 snapshot 隔离、多 mutation 乱序结束、迟到 finish、heartbeat freshness、lease loss、stalled/failed/ambiguous/source-conflict copy cleanup、destination lease/source-delete 竞态、rebuild/mutation 竞态、pending copy、session-stale Cosmos 拒绝、查询中 fence ETag 漂移、malformed/future/expired fence、ready marker 回滚、stale workspace、partial UI、complete-only cache、busy fail-closed 和旧 Backend 数组兼容。
2.94a 发布、清理与隔离补充契约：首次上线必须先以 `writers-only` phase 发布全部 fence-aware writer，同时让 page 请求返回可回退的 `photo-catalog-not-ready`，且 legacy 请求不得读取或 materialize catalog；exact-SHA Backend 健康后至少等待 10 分钟，再用独立提交切到 `enabled`。回滚必须先禁用 catalog read/publication，不能直接恢复 legacy writer。snapshot 批量写首错后必须停止派发并等待全部已启动 worker settle，再进入 abandon/cleanup，禁止迟到 upsert 在删除后复活旧 snapshot。ready legacy 读取、成功 rebuild 和每个 revision 的首 page 都必须在 Blob ready fence 前后未变化时，每次最多删除 250 条非 active catalog 行；只有本轮少于上限才可记忆为 clean，失败或达到上限必须由后续首 page 续清，查询期间出现的新 rebuild 不得被清理。位置查询与列表 hydration 必须同时要求 `NOT IS_DEFINED(c.docType)` 和数值 `lat/lon`，并在应用层拒绝任何带 `docType` 的共享容器文档，防止同名 catalog 行污染位置去重。cold-load 只读预览必须在授权 thumbnail、preview 与 direct/proxy 候选间使用既有有界媒体回退，全部失败后才显示本地占位。
2.95 Backend exact-SHA 生产身份契约：`deploy-backend.yml` production jobs 只能在 `refs/heads/main` 运行；非 main dispatch 必须使用独立可取消 group，main 共用不取消 active run 的 production group。发布严格串联 preflight → unprivileged build/package → final authorization → privileged deploy/receipt；只有 preflight/authorization 拥有 Actions write 并仅把 `GITHUB_TOKEN` 传给 target step，只有 deploy 拥有 `id-token: write`，build 不得取得两者；权限 map 必须精确且不能加入未知 write scope。静态 parser 只允许已覆盖的 block-style YAML 子集，对重复/显式/flow mapping、anchor、alias 与 escaped key fail closed。两次 target checkout 必须 `fetch-depth: 0`，刷新 `origin/main` 后要求 triggering `GITHUB_SHA` 是 main ancestor，且 `GITHUB_SHA..origin/main` 在 Backend artifact、根 `package.json`/`yarn.lock`、deploy/health workflow 和发布运行脚本的精确 trigger/pathspec 集合中没有 commit；只有无关前端/文档前移可继续。目录的 GitHub trigger 必须保留递归 `/**`，并与对应 Git pathspec 由同一列表生成；stale run、分叉历史或 build 期间出现新 Backend-relevant commit 必须 fail closed，并在 scoped Actions write 下重新 dispatch 当前 main，防止替换唯一 pending 后无人接替。Backend build 在无特权 job 执行 target、production smoke、workflow parser 行为测试与静态 checker，生成唯一 1 天 artifact；deploy 下载后必须在 OIDC 前验证 artifact 只含一个 root marker 且 SHA exact，并在 Azure upload 同一步重算 SHA-256 以证明验证后的包未被替换。部署 zip 根目录写入 raw canonical、lowercase 40 位 SHA 的 `deployment.json`，仅允许可选末尾换行，重复 `sha`、额外字段或替代序列化格式都拒绝；匿名 `GET /api/deployment` 每次从部署根读取，只允许该 marker，成功返回精确 JSON + 仅 `Cache-Control: no-store`，缺失、malformed、非 canonical 时返回 no-store 503 且不向客户端泄露路径或解析错误。Azure upload 后必须无条件运行 `backend-deployment` scope，使用 cache-busting query 并发验证主域、`www` 和 Functions 直连三条 marker，禁用 redirect，最多 8 轮有限重试；receipt 与 controller identity step 禁止 `if:false`/`continue-on-error` 绕过。Production Health classifier 固定 attempt-specific jobs；零 job、skipped job 或 skipped upload 不算 deployment started，但 main 上成功结束且未部署的 Backend run 必须 fail closed；只有真实 upload+receipt 都成功才接受 Backend event。deployed revision full smoke 与 controller-owned marker-only gate 都接收 `PRODUCTION_BACKEND_DEPLOYED_SHA`，三条 marker 必须 exact/no-store。普通 smoke 为 23 项，Frontend 或 Backend exact-SHA full smoke 各 26 项。GitHub 历史 rerun 使用原提交中的 workflow，本契约不能追溯保护上线前 run；旧 run 不得 rerun，误触发后必须由当前 main dispatch 恢复。
2.95a Backend 生产依赖身份契约：build/test 与 deploy-stage 必须消费同一根 `yarn.lock`；后者只带 server manifest，并在根 frozen install 已填充的 cache 上执行 production-only、offline、frozen Yarn install，禁止无 lock 的 npm caret 解析。打包前必须遍历部署 `node_modules` 全部嵌套 package identity，证明每个 `name@version` 都在已测试图中，并证明 server 每个直接依赖存在；缺少、额外未测试版本或 graph checker 被移除均 fail closed。手工加入的 Windows sharp tarball 必须由 lock 中唯一精确 selector 提供相同 version 与 SHA-512 integrity，digest 不符不得解包。workflow 静态契约必须锁定完整打包命令、graph checker、离线冻结参数、tarball digest gate 和对应 Backend runtime trigger；这些脚本变化必须使历史 deployment stale。
2.95b Backend partial-rerun fence：不得假设成功的 upstream authorization 会在 GitHub “rerun failed jobs” 中再次执行。deploy job 必须使用第二个隔离 checkout 取得完整历史且 `persist-credentials: false`，在 Azure login 前和紧邻 `config-zip` 前各运行一次无 requeue flag、无 `GH_TOKEN` 的 target checker。两次均要求原 `GITHUB_SHA` 仍是 `origin/main` ancestor 且之后无 Backend-relevant commit；任何失败都发生在 upload 前。只有 preflight/authorize 保留 Actions write 和自动 requeue，deploy 继续只有 contents read + OIDC，禁止为方便恢复而合并两种 write 权限。静态契约必须锁定两个 deploy-local step 的 job、命令、working directory、顺序、full-history checkout 和零 GH token，并以删除、移动到 upload 后、恢复持久凭据或加入 `--requeue-current` 的破坏性 fixture 证明会失败。上线前历史 workflow 不具备此步骤，仍不得 rerun。
2.96 `www` 登录注册与生产 CORS 契约：`registrationApi.ts` 的 unsafe `POST /auth/register` 必须始终只发送一次。默认 SWA hostname 继续固定使用 `PROXY_API_BASE`；`www.cloudphotos.top` 必须从启动时 `API_BASE` 开始，让共享 same-origin probe 在 Nginx 落点改写到当前 origin `/api`，在 SWA 落点保留 Azure Functions 直连，禁止先失败再重放注册。登录维持相同 `www` 落点选择和仅对幂等认证语义开放的串行 fallback。Backend OIDC job 必须在最终 deploy-local target fence 前幂等加入 `https://cloudphotos.top`、`https://www.cloudphotos.top` 与精确 SWA 默认 origin，回读确认三者存在并拒绝 `*`；`host.json` 使用同一非通配集合。Production smoke 必须以固定不存在账号验证 login 获得受控 401/404、以 `{}` 验证 register 获得 JSON 400，并分别模拟 `www`→Functions 与 SWA→主域代理的 OPTIONS，要求 exact ACAO 和 `Content-Type` allow-header；探针不得使用真实凭据或创建账号。workflow/source 行为测试必须覆盖遗漏 `www`、将 CORS 移到最终 target fence 后、Nginx/SWA 两种 `www` 落点、注册一次调用以及 preflight 缺失/错 origin/缺 header。
2.97 注册 lazy chunk 失败隔离契约：`AuthPage.tsx` 必须继续通过同一个缓存 Promise 和 `React.lazy` 按 hover/focus/click intent 加载独立 `RegisterForm` chunk，不得将注册字段、校验或提交逻辑静态带回登录入口。loader 必须同时处理两种失败：普通 dynamic import rejection 在内部交给 `reportLazyBoundaryFailure` 后返回 fallback；全局 `vite:preloadError` 被 deployment recovery `preventDefault()` 后，Vite helper 可能把 import Promise 解析为 `undefined`，fulfilled handler 也必须替换为 fallback，不能让 `React.lazy` 读取 `undefined.default`。预载失败不得成为未处理 rejection 或越过 `AuthPage` 触发整个“登录与注册”错误边界；同一 rejected Promise 只本地上报一次，Vite-suppressed 分支复用全局已完成的上报。fallback module 必须与真实注册表单同契约：inactive 时保持隐藏，active 时只替换注册 panel、显示注册专用错误，并通过现有 `requestDeploymentRefresh` 提供“刷新新版”；登录 tab、输入与提交保持可用。`auth-layout-contracts.test.mjs` 必须执行真实 transpiled loader，分别注入 rejected import 与 fulfilled-undefined preload 结果，证明两次调用复用同一 fallback、上报次数准确、inactive 隐藏和 active 恢复按钮；静态契约与构建检查继续证明唯一 RegisterForm deferred chunk、无 HTML preload/SW precache，并维持不可放宽的 36,000 B raw 登录入口门禁。

2.98 登录首屏、PWA 检查与群组管理性能契约：`AuthContext` 无 token 分支必须先保存 `clearPrivatePhotoCaches()` Promise；该调用的同步段必须立即递增 generation、清空 owner、通知内存 reset 并删除作用域键，然后才允许 `setLoading(false)` 显示认证 UI。完整 Workbox/Cache Storage/IndexedDB 清理仍须在后台 `await` 并显式记录失败，后续登录/恢复的 scoped preparation 必须等待共享清理链；logout、401、切号和角色变化路径继续等待完成，禁止以 fire-and-forget 换取首屏。`main.tsx` 必须在 React root 前安装 deployment recovery，但只能在 render 后通过 document-complete 或一次 `load`，再以 `requestIdleCallback({timeout:2000})`/`setTimeout(0)` 注册 Service Worker；`onRegisteredSW` 不得重复首次 `registration.update()`，standalone 判定也必须留在延迟加载的 policy chunk，禁止带回登录入口。独立 `updateCheckPolicy` 必须动态导入、进入最小 precache 且不进入 HTML preload，只在 visible + online + no-inflight 时检查；standalone/browser 周期固定 5/15 分钟，focus/visibility 非强制检查至少间隔 60 秒，online 可强制一次但仍受可见性和 in-flight 去重，同步抛错和异步 rejection 都必须释放 in-flight 并允许重试，dispose 必须清理 interval/listeners。`CreateGroupDialog` 与 `GroupSettings` 必须从初始 `AuthenticatedApp` chunk 拆出，各自复用缓存 import Promise 并在菜单 hover/focus 预热；普通 rejection 要上报，Vite fulfilled-undefined 要替换为合法 recovery module。Suspense loading 与 recovery fallback 都必须使用 modal focus boundary，关闭按钮为初始焦点，并支持 Esc、遮罩关闭和“刷新新版”，不能因 chunk 故障锁住已登录工作区。构建合同必须证明两组对话框无 HTML preload/SW precache、登录入口不超过 36,000 B；当前同条件 production preview 基线为 FCP/LCP 1712→1376ms、登录表单约 988ms、CLS=0，production-env 入口 35,867 B，`AuthenticatedApp` 输出 167.61→161.05 kB（gzip 54.65→52.95 kB）。发布后 retained-entry 三次 fresh-profile 对照要求记录登录可见、LCP、首轮 `/assets/` 传输与 PWA 时序；当前登录可见 1454→1024ms、LCP 1976→1564ms，首轮 app asset 73,214→74,109 B，换取持续 update 请求大幅降频。
2.99 封面缺失、慢速加载与骨架反馈契约：`selectGridMediaSources()` 必须保持被动 derivative-only，时间线、重点片段、文件夹、故事和预览网格不得因缺少 thumbnail/preview 自动下载 original。`selectGridRetrySources()` 只有在用户明确点击卡片错误态后，才允许普通静态、非 HEIC/HEIF 图片把 original 追加为最后候选；GIF、Motion Photo、其他 animated media 和 HEIC/HEIF 必须继续拒绝该 fallback。原图重试只携带 `cf_cover_retry`，不得携带 `cf_cover=1` 或写入 derivative private cache。PhotoCard 只在前 6 张 priority 或卡片进入 `IntersectionObserver(rootMargin:"600px 0px")` 后挂载真实来源；全局最多 6 个 cover source attempt，同一卡片每个获准来源拥有独立 8 秒预算，不能按候选数平分，另以 18 秒 wall-clock 总边界结束异常 loading。排队卡片在获准前只渲染 `BLANK_GIF` 与 skeleton，deadline 必须和真实请求 admission 对齐；成功、error、timeout、unmount、scope/context 变化都必须释放 slot，迟到 callback 需同时验证 auth role、照片、retry generation、候选和当前 DOM element。Service Worker 尚未 controller/active 时，`enablePrivateCacheWrites()` 必须返回 `deferred` 而非 success；lazy `privateCacheReset` chunk 负责监听 `ready` 和 `controllerchange`、合并并发 replay，静态 lifecycle 只保存 auth scope、generation 和取消句柄。注销、401、账号或角色变化必须取消旧 enable；未 enable 期间媒体网络继续可用，但私有 CacheStorage 读写 fail closed。首次 catalog 等待、完整图库分页、PhotoGallery/FolderView lazy fallback 固定显示 8 张 `.photo-grid-skeleton-card`，渐进 preview 与每张真实封面在 decode 前显示 skeleton、成功后淡入、失败后移除 loading 并显示可重试状态；shimmer 必须有足够可见对比并在 `prefers-reduced-motion` 下停用。浏览器回归必须覆盖延迟 catalog、4.5 秒成功且 thumbnail-only、缺派生图零被动原图请求与显式 retry、网络失败、无限 body stall、cache disabled、logout/切号、late fence release，并继续通过不可放宽的 36,000 B raw 登录入口门禁。
...
- deploy-backend.yml
1. 仅 server 相关变更触发
2. preflight full-history checkout 在无 OIDC 权限下拒绝 stale SHA，必要时重新 dispatch 当前 main
3. unprivileged build job 安装、构建、测试并生成含 canonical deployment.json 的单一短期 artifact
4. final authorization 再次拒绝 build 期间出现的 Backend-relevant 前移并负责 requeue
5. 唯一 OIDC job 下载/复核 artifact 后串行部署到 Function App
6. 主域、www 与 Azure 直连 `/api/deployment` 精确回读同 SHA 后才记录 canonical receipt
