...
**紧急修复**
- **🔐 登录线路串行补偿** — `cloudphotos.top` 同源登录 transport 或 retryable gateway 失败后才尝试一次 Azure；登录不并发 hedge，注册、上传、分享创建和其他写请求仍绝不重放，两条线路均失败时不再误报只有代理故障
- **⬆️ 未登录也可立即更新** — 登录页监听已存在与新到达的 PWA update-ready 状态，旧 Service Worker 客户端可在认证前显式激活等待版本；missing updater、操作阻塞和更新异常均显示可访问反馈，安装入口保持不变
...
- **🎯 Production Health 部署身份** — `workflow_run` checkout、分类、报告与线上 guard 统一绑定 triggering deployment `head_sha`，jobs 分类同时固定到该事件的 `run_attempt`；前端 artifact 发布 no-store SHA marker，主域、`www` 和 SWA 必须精确匹配且不得重定向，避免 main 已前移、入口漂移或同 run 重跑时造成假绿/假红
