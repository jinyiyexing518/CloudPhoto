...
**Bug 修复**
- **🎬 视频播放中途黑屏/无限加载** — 时间线与文件夹查看器复用同一恢复控制器；视频线路改用 2-byte Range/206 探测，播放态连续 4 秒无时间进展才有限换线，并在新线路 metadata 后恢复原位置、播放意图、静音、音量与倍速。跨域 Blob 备用播放不依赖存储 CORS；暂停、seek、后台、结束和旧 viewer 事件不会误切，双线路失败会结束 spinner 并提供原位重试
