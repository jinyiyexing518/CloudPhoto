 ...
 */
export async function preparePrivatePhotoCachesForOwner(cacheOwner: string): Promise<void> {
  if (typeof window === "undefined" || !cacheOwner) return;
  ...
  if (owner !== cacheOwner) await clearPrivatePhotoCaches();
  await cleanupChain;
  try {
    localStorage.setItem(CACHE_OWNER_KEY, cacheOwner);
  } catch {
    // Authorization-scoped cache keys still isolate memory and Cache Storage.
  }
}
